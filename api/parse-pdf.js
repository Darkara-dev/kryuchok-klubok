...
6. Если у детали есть общее указание (например "нить А, набивать по ходу вязания") — вынеси в поле meta детали. ВАЖНО: meta — это только КОРОТКАЯ общая пометка (цвет нити, общий совет по набивке и т.п.), которая относится ко ВСЕЙ детали целиком. Никогда не копируй туда текст конкретного ряда (в том числе первого) — содержание каждого ряда должно быть только в его собственном description, а не повторяться в meta.
...
9. Порядок деталей в ответе должен ТОЧНО повторять порядок, в котором они идут в исходном документе — не переставляй детали местами, даже если тебе кажется, что логичнее было бы связать их в другом порядке. Автор схемы располагает детали в порядке вязания не просто так (например, деталь "Подкладка шляпки" идёт раньше детали "Шляпка", потому что шляпка довязывается прямо на край уже готовой подкладки, — переставлять их местами нельзя).

10. Переводи не дословно, а по смыслу — сначала пойми, что мастерица физически делает руками в этом ряду, и только потом сформулируй понятную русскую инструкцию. Особенно это важно, когда ряд описывает действие с УЖЕ ГОТОВОЙ другой деталью (например "провязать в край предыдущей детали", "ввести крючок в такой-то ряд другой детали и провязать разное количество петель в зависимости от типа этого ряда") — дословный перевод таких мест обычно получается непонятным набором слов. В таких случаях поясни своими словами, что нужно сделать, не теряя техническую точность (числа петель, тип столбика), и при необходимости вынеси уточнение в note.

8. ЧЕРЕДУЮЩИЕСЯ ПАРЫ РЯДОВ (важно, частая ошибка): если в оригинале сказано "повторять ряды X и Y N раз" (то есть ряды ЧЕРЕДУЮТСЯ между собой — сначала X, потом Y, потом снова X, потом снова Y, и так N раз каждый) — это НЕ то же самое, что "repeat" у одного ряда. НЕЛЬЗЯ сворачивать это как {ряд X, repeat: N} и {ряд Y, repeat: N} по отдельности — так теряется чередование и порядок рядов становится неправильным. Вместо этого используй отдельный формат — объект-группу вместо обычного ряда:
