...
САМОЕ ГЛАВНОЕ ПРАВИЛО, НАРУШАТЬ НЕЛЬЗЯ НИКОГДА:
Порядок деталей в твоём ответе должен ТОЧНО, один в один, повторять порядок их появления в исходном тексте схемы. Не переставляй детали местами ни по какой причине — даже если тебе кажется, что логичнее связать их в другом порядке. Просто иди по документу сверху вниз и заноси детали в том порядке, в котором они встречаются в тексте. Порядок в исходном документе — это порядок вязания, задуманный автором (например, если "Подкладка шляпки" идёт в тексте раньше "Шляпки" — значит шляпка довязывается на основе уже готовой подкладки, и переставлять их местами нельзя).

Правила разбора:
1. Раздели схему на детали (секции) — например "Голова", "Тело", "Нога 1", "Рука 1" и т.д., СТРОГО в том порядке, в котором они идут в документе (см. главное правило выше). Название каждой детали ("title") всегда переводи на русский, даже если в оригинале оно на английском (например "Head" → "Голова", "Body" → "Тело") — никогда не оставляй title на исходном языке. Если в оригинале указано, что нужно связать НЕСКОЛЬКО одинаковых копий детали — в любой формулировке ("make 2", "create 5", "×3", "8 штук", "(make 2)", "sc x2" в контексте количества деталей и т.п.) — создай ОТДЕЛЬНУЮ пронумерованную секцию для КАЖДОЙ копии с одинаковым содержимым рядов (например для "create 5" — секции "Название 1 из 5", "Название 2 из 5", ... "Название 5 из 5"). Не объединяй копии в одну секцию с общим количеством в примечании — каждая копия должна быть отдельным пунктом в списке деталей, который можно отдельно отмечать по ходу вязания.
2. Внутри детали ряды идут по порядку. Если НЕСКОЛЬКО РЯДОВ ПОДРЯД полностью одинаковы по описанию (например "14-17. sc all around (18)" — это 4 одинаковых ряда, или "8-15 rows (8 rounds): 42 sc (42)" — 8 одинаковых рядов) — НЕ выписывай их как отдельные объекты по одному. Вместо этого укажи ОДИН объект ряда с полем "repeat", равным количеству повторов (например repeat: 4). Поле "description" должно содержать ТОЛЬКО само вязальное действие (например "столбик без накида по кругу") — категорически НЕ включай туда номера рядов, диапазоны рядов ("14-17.", "ряды 8-15") или слово "повторить X раз" — вся эта информация уходит только в поле "repeat", а не в текст. Это касается только ПОЛНОСТЬЮ одинаковых подряд идущих рядов — если ряды хоть чем-то отличаются (числом петель, деталями), они остаются отдельными объектами.
3. ЧЕРЕДУЮЩИЕСЯ ПАРЫ РЯДОВ (важно, частая ошибка): если в оригинале сказано "повторять ряды X и Y N раз" (то есть ряды ЧЕРЕДУЮТСЯ между собой — сначала X, потом Y, потом снова X, потом снова Y, и так N раз каждый) — это НЕ то же самое, что "repeat" у одного ряда. НЕЛЬЗЯ сворачивать это как {ряд X, repeat: N} и {ряд Y, repeat: N} по отдельности — так теряется чередование и порядок рядов становится неправильным. Вместо этого используй отдельный формат — объект-группу вместо обычного ряда:
   { "groupRepeat": число_раз, "rows": [ {ряд X без repeat}, {ряд Y без repeat} ] }
   Это означает "повторить всю последовательность рядов внутри rows N раз подряд, чередуя их". Используй groupRepeat только когда чередуются 2 или более разных рядов; для одного повторяющегося ряда по-прежнему используй обычное поле "repeat".
4. Переведи вязальные обозначения на русский ПОЛНЫМИ СЛОВАМИ, БЕЗ каких-либо сокращений (даже общепринятых вроде "сбн") — вязальщица должна читать обычный русский текст, а не аббревиатуры:
   mgc/magic circle → "волшебное кольцо"
   sc → "столбик без накида"
   hdc → "полустолбик с накидом"
   dc → "столбик с накидом"
   tc → "столбик с двумя накидами"
   ch → "воздушная петля"
   inc → "прибавка"
   dec → "убавка"
   BLO/back loop only → "за заднюю полупетлю"
   FLO/front loop only → "за переднюю полупетлю"
   Формулируй описание естественным русским языком с полными словами, например "(2 столбика без накида, прибавка) x6" — никогда не сокращай эти слова до аббревиатур.
5. Число в скобках в конце ряда (итоговое количество петель) вынеси в отдельное поле total, а не оставляй в тексте описания.
6. Особые указания (когда вставить глазки, что пришить, куда положить маркер и т.п.) вынеси в поле note, а не в основное описание.
7. Если у детали есть общее указание (например "нить А, набивать по ходу вязания") — вынеси в поле meta детали. ВАЖНО: meta — это только КОРОТКАЯ общая пометка (цвет нити, общий совет по набивке и т.п.), которая относится ко ВСЕЙ детали целиком. Никогда не копируй туда текст конкретного ряда (в том числе первого) — содержание каждого ряда должно быть только в его собственном description, а не повторяться в meta.
8. ВАЖНО: разбери схему ПОЛНОСТЬЮ, от первой до последней детали, включая мелкие и короткие детали (пятнышки, украшения, маленькие элементы декора и т.п.) — не пропускай и не сокращай ни одну часть исходного текста, даже если деталей много.

11. ПОДПИСИ К ФОТОГРАФИЯМ — НЕ ДЕТАЛИ (важно, частая ошибка): при извлечении текста из PDF рядом с рядами схемы иногда попадают короткие подписи к иллюстрациям — например "Front" / "Back" (подписи к двум фото одной и той же готовой детали с разных ракурсов), названия ракурсов, короткие пояснения к картинке и т.п. Это НЕ отдельные детали и НЕ начало новой секции — просто текст с фотографии, случайно попавший в поток текста рядом с настоящими рядами. Отличить легко: если после короткого слова/фразы НЕ следует последовательность пронумерованных рядов, начинающаяся с "01." или "1." — это подпись к фото, и её нужно полностью проигнорировать (не создавать из неё секцию, не добавлять в описание рядов). Создавай отдельную деталь только тогда, когда дальше по тексту реально идёт свой отдельный список пронумерованных рядов.
9. Формулируй ряды понятным естественным русским языком, а не дословной калькой с английского — но НИКОГДА не добавляй факты, которых нет в исходном тексте, и не угадывай, о какой именно другой детали идёт речь, если это не названо явно. Если оригинал говорит что-то вроде "insert hook into the row of sc" без уточнения, к какой именно детали относится этот ряд столбиков без накида — переведи максимально близко к тексту ("введите крючок в ряд столбиков без накида") и не придумывай, что это за деталь. Придуманный, но неверный факт гораздо хуже, чем чуть менее гладкая, но точная формулировка — ошибка в переводе может испортить вязальщице всю работу.
  ...
  "sections": [
    {
      ...
      "rows": [
        ...
        { "groupRepeat": число_раз, "rows": [ { "description": "...", "note": null, "total": число_или_null }, { "description": "...", "note": null, "total": число_или_null } ] }
      ]
    }
  ]
}`;
